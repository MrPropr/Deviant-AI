"""Create placeholder local-model outputs without downloading model weights."""
...
def build_outputs(records: list[dict], model_id: str, run_id: str) -> list[dict]:
    outputs: list[dict] = []
    for record in records:
        outputs.append(
            {
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output",
        type=output_file,
        default=Path("results/raw/local_model_outputs.jsonl"),
        help="Raw output JSONL path. Use an ignored path for real outputs.",
    )
    parser.add_argument("--model-id", default="placeholder-local-model", help="Model identifier for the run.")
    parser.add_argument("--run-id", default="local_placeholder_run", help="Run identifier.")
...
def main() -> None:
    ...
    outputs = build_outputs(records, args.model_id, args.run_id)
    write_jsonl(args.output, outputs)
    print(
        f"Wrote {len(outputs)} placeholder local-model outputs at {args.output}. "
